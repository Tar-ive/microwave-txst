...
// Site-wide report stats. weeklyReports blends a seeded baseline number with
// real user-submitted reports (tagged source: "user" by the API) so the
// displayed count becomes organic as real usage grows.
export function computeStats(db) {
  const baseline = db.stats?.weeklyReportBaseline ?? 0;
  const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  let realWeekly = 0;
  let realTotal = 0;
  for (const mw of db.microwaves) {
    for (const r of mw.reports || []) {
      if (r.source === "user") {
        realTotal++;
        if (new Date(r.at).getTime() >= weekAgo) realWeekly++;
      }
    }
  }
  return {
    weeklyReports: baseline + realWeekly,
    realWeeklyReports: realWeekly,
    realTotalReports: realTotal,
  };
}
